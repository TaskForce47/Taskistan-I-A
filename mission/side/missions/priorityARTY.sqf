...
//-------------------- 2. SPAWN OBJECTIVES
	...
	priorityObj1 = "rhsgref_ins_g_BM21" createVehicle _flatPos1;
	waitUntil {!isNull priorityObj1};
	priorityObj1 setDir _PTdir;
	
	sleep 0.3;
	
	priorityObj2 = "rhsgref_ins_g_BM21" createVehicle _flatPos2;
	...
	ammoTruck = "rhs_gaz66_ammo_vv" createVehicle _flatPos3;
...
//-------------------- 3. SPAWN CREW
	...
	_priorityGroup = createGroup east;
		...
		"rhsgref_ins_g_squadleader" createUnit [_flatPos, _priorityGroup];
		"rhsgref_ins_g_squadleader" createUnit [_flatPos, _priorityGroup];
		"rhsgref_ins_g_crew" createUnit [_flatPos, _priorityGroup];
		"rhsgref_ins_g_crew" createUnit [_flatPos, _priorityGroup];
...
//-------------------- 5. SPAWN FORCE PROTECTION
	...
	_firingMessages = [
		...
		"Enemy Artillery rounds incoming! Seek cover immediately!",
...
while { canMove priorityObj1 || canMove priorityObj2 } do {
		...
		while {!_accepted} do {
			...
			if (!isNull _unit) then {
				...
				if ((_targetPos distance (getMarkerPos "respawn_west")) > 5000 && vehicle _unit == _unit && side _unit == WEST) then { 
					_accepted = true; 
				} else {
					sleep 7;																// default 10
				};
		...
		
		{
			if (alive _x) then {
				for "_c" from 0 to 2 do {
					...
					_x doArtilleryFire [_pos, "rhs_mag_40Rnd_122mm_rockets", 1];
					sleep 4;															// default 8
				};
			};
