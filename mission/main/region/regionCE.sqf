...
Notes:
	...
	_targets = ["Chak Chak","Feruz Abad","Falar","Garmarud","Garmsar","Mulladost","Khushab","Shurkurkalay","Charman","Sakhe","Timurkalay","Zavarak","Bastam","Nur","Karachinar","Nagara"];
...
while { count _targetArray > 0 } do {
		...
		[] spawn {
			...
			if (alive radioTower) then {
				while {(alive radioTower)} do {
					...
					["aoMarker"] call QS_fnc_enemyQrfAir;
					sleep 8000;
				};
			};
		};
	...
	["tf47_changetickets", [WEST, 2, 30]] call CBA_fnc_globalEvent;
